...
function createPlugin(regexp, replacer, plugin_id) {
  ...
  if (plugin_options && typeof plugin_options.shouldParse === 'function') {
    if (!plugin_options.shouldParse(state)) {
      return false
    }
  }
  ...
  function render(tokens, id, options, env) {
    return replacer(tokens[id].meta.match, { escape }, plugin_options, tokens, id, env);
  }
